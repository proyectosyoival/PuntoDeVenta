...
<body>
  ...
  <?php include 'views/mensajes.php'; ?>
    ...
    <div class="text-left" id="respuesta"></div>
		<h1 class="text-left" id="h1-tab-titulo">ROLES</h1>

		<div>
			<a type="button" class="btn" id="btn-table" href="<?php echo constant('URL'); ?>rol/nuevo"><span class="icon-plus"></span> Nuevo</a>
		</div>
        <hr>
            ...
            <tbody id="tbody-general">
                <?php
                    ...
                    foreach($this->roles as $row){
                        ...
                        $rol = $row; 
                ?>
                <tr id="fila-<?php echo $rol->id_rol; ?>">
                    <!-- <td><?php echo $rol->id_rol; ?></td> -->
                    <td><?php echo $rol->nombreRol;?></td>
                    <td><?php echo $rol->descripcionRol;?></td>
                    <td><?php $fecha=$rol->fecha_alta; $date = date("d/m/Y H:i:s", strtotime($fecha)); echo $date;?></td>
                    <td><a type="button" class="btn" id="btn-editar" href="<?php echo constant('URL') . 'rol/verRol/' . $rol->id_rol; ?>"><span class="icon-pencil2"></span></a></td>
                    <td><a type="button" class="btn btn-danger bEliminar" data-id="<?php echo $rol->id_rol;?>" data-function="rol/eliminarRol"><span class="icon-bin"></span></a></td>
                </tr>
                <?php } ?>
            </tbody>
        </table>
